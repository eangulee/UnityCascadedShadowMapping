...
Shader "Custom/ShadowMap/ShadowMapNormal"
{
	Properties
	{
		...
		SubShader
	{
		Pass
		{
			...
			float4x4 _gProjectionMatrix;
			sampler2D _gDepthTexture;
			...
			v2f vert(appdata_full v)
			{
				...
				//动态阴影
				o.depth = o.pos.zw;
				_gProjectionMatrix = mul(_gProjectionMatrix, unity_ObjectToWorld);
				o.proj = mul(_gProjectionMatrix, v.vertex);
				//--------------------------------------------------
			...
			fixed4 frag(v2f v) : COLOR
			{
				...
				fixed4 dcol = tex2Dproj(_gDepthTexture, v.proj);
